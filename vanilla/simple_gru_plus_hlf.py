...
def build_GRU_HLF_model(name, input_width,out_width, depth,recurrent_width, lstm_activation="tanh", lstm_dropout = 0.0,
    ...
    dense_out = Dense(recurrent_width, activation="relu", name='GRU_output')(a)
    ...
    aHLF = Dense(100, activation='relu')(aHLF)
    aHLF = Dense(50, activation='relu')(aHLF)
    aHLF =  Dropout(dropout)(aHLF)
    aHLF = Dense(25, activation='relu', name='HLF_output')(aHLF)
    aHLF = Merge(mode='concat',concat_axis=1)([dense_out, aHLF])
    if(dropout > 0.0):
        aHLF =  Dropout(dropout, name="dropout_final")(aHLF)
    dense_final = Dense(out_width, activation=output_activation, name='Final output')(aHLF)
...
# training set:
train = gen_from_data(train_dir, batch_size=200, data_keys=[["Particles","HLF"], "Labels"])

# validation set:
val = gen_from_data(val_dir, batch_size=200, data_keys=[["Particles","HLF"], "Labels"])
...
history = model.fit_generator(train, samples_per_epoch=train_samples, 
                            nb_epoch=50,
                           validation_data = val, 
...
with open("simple_GRU_HLF_last.json","w") as js:
    js.write(model_json)

model.save_weights("simple_GRU_HLF_last_weights.h5")
